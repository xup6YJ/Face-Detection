...
#Read only data
trainbbx_path = 'WireFace/wider_face_split/wider_face_train_bbx_gt.txt'
bbxfile = read.table(trainbbx_path, sep="\t")
...
save(all_names, file = 'WireFace/annotation/all_names.RData')
...
for (i in 1:nrow(annotation)) {
  ...
  if(i %% 1000 == 0){print(i)}
  img = readJPEG(paste0(img_dic, '/', annotation[i,'img_id']))
  annotation[i,'org.width'] = dim(img)[1]
  annotation[i,'org.height'] = dim(img)[2]
  
}

save(annotation, file = 'WireFace/annotation/annotation.RData')