...
model_AP_func <- function (model, Iterator, ctx = mx.gpu(), IoU_cut = 0.5) {
  ...
  num_batch <- 1
  batch_per_epoch <- floor(length(val_ids)/16)
  ...
  while (Iterator$iter.next()) {
    ...
    label_result <<- Decode_fun(label, cut_prob = 0.5, cut_overlap = 0.5)
    label_box_info[[num_batch]] <- label_result
    label_box_info[[num_batch]]$img_id <- label_box_info[[num_batch]]$img_id + (num_batch - 1) * dim(img_array)[4]
    
    mx.exec.update.arg.arrays(val_exec, list(data = img_array), match.name = TRUE)
    mx.exec.forward(val_exec, is.train = FALSE)
    
    predict_result <<- Decode_fun(val_exec$ref.outputs[[1]], cut_prob = 0.5, cut_overlap = 0.3)
    pred_box_info[[num_batch]] <- predict_result
...
val_annotation = new_val_annotation
...
my_iterator_core <- function (batch_size, 
  ...
  value <- function() {
    ...
    for (i in 1:batch_size) {
      ...
      # i = 1
      sub_file <<- train_box_info[train_box_info$img_id %in% train_ids[idx[i]],]
      filename <<- paste0(img_dic, '/', sub_file$img_id[1])
      read_img <- readJPEG(filename)
      
      if(train_val == 'train'){
        # dim(read_img)
        #height, width
        img = resizeImage(read_img, sub_file$resize_height[1], sub_file$resize_width[1], method  = 'bilinear')
        img = img[sub_file$h1[1]:sub_file$h2[1], sub_file$w1[1]:sub_file$w2[1], ]
        dim(img) = c(dim(img)[1], dim(img)[2], 3, 1)
      }else{
        
        img = resizeImage(read_img, img_size, img_size, method  = 'bilinear')
      }
